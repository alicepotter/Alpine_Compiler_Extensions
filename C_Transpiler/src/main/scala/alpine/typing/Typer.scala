...
import alpine.typing.Solver.apply
...
final class Typer(
  ...
  def visitFunction(d: ast.Function)(using context: Typer.Context): Type =
    ...
    val functionType: Type = context.inScope(d, { inner =>

      val computedBodyType = memoizedUncheckedType(d, (func) => computedUncheckedType(func)(using inner))
      val verifiedBodyType = checkedType(d.body)(using inner)
      val typee = if(d.output.isEmpty) then Type.Unit else evaluateTypeTree(d.output.get)
      checkInstanceOf(d.body, typee)
      context.obligations.constrain(d, computedBodyType)
      computedBodyType

    })
    
    val result = if functionType(Type.Flags.HasError) then Type.Error else functionType
  ...
  def visitRecord(e: ast.Record)(using context: Typer.Context): Type =
    val fieldsType = e.fields.map(l => Labeled(l.label, l.value.visit(this)(using context)))
    context.obligations.constrain(e, Type.Record(e.identifier, fieldsType))
  ...
  def visitSelection(e: ast.Selection)(using context: Typer.Context): Type =
    ...
    var res :Type = Type.Unit
    
    e.selectee match
      case s: ast.Identifier =>
        q match
          case Type.Record(index, fields) => 
            fields.filter(field => field.label.getOrElse("") == s.value) match
              case Nil => Type.Error
              case head :: next => 
                val selectedType = head.value
                context.obligations.constrain(e, selectedType)
                context.obligations.add(Constraint.Member(q, selectedType, s.value, e, Constraint.Origin(e.site, None)))
                selectedType
          case _ => Type.Error    
      ...
      case s: ast.IntegerLiteral =>
         q match
          case Type.Record(index, fields) => 
            evaluateFieldIndex(s).getOrElse(Int.MaxValue) match
              case index if index < fields.size => 
                val selectedType = fields(index).value
                context.obligations.constrain(e, selectedType)
                context.obligations.add(Constraint.Member(q, selectedType, index, e, Constraint.Origin(e.site, None)))
                selectedType
              case _ => Type.Error  
          case _ => Type.Error
  ...
  def visitApplication(e: ast.Application)(using context: Typer.Context): Type =
    val argumentsType = e.arguments.map(l => Labeled(l.label, l.value.visit(this)(using context)))
    val functionType = e.function.visit(this)(using context)
    
    Type.Arrow.from(functionType) match
      case None => 
        val fresh = freshTypeVariable()
        context.obligations.add(Constraint.Apply(functionType, argumentsType, fresh, Constraint.Origin(e.site)))
        context.obligations.constrain(e, fresh)
      case Some(value) =>
        context.obligations.add(Constraint.Apply(functionType, argumentsType, value.output, Constraint.Origin(e.site)))
        context.obligations.constrain(e, value.output)
    

  def visitPrefixApplication(e: ast.PrefixApplication)(using context: Typer.Context): Type =
    val argumentType = e.argument.visit(this)(using context)
    val functionType = e.function.visit(this)(using context)

    Type.Arrow.from(functionType) match
      case None =>
        val fresh = freshTypeVariable()
        context.obligations.add(Constraint.Apply(functionType, List(Labeled(None, argumentType)), fresh, Constraint.Origin(e.site)))
        context.obligations.constrain(e, fresh)
      case Some(value) =>
        context.obligations.add(Constraint.Apply(functionType, List(Labeled(None, argumentType)), value.output, Constraint.Origin(e.site)))
        context.obligations.constrain(e, value.output)


  def visitInfixApplication(e: ast.InfixApplication)(using context: Typer.Context): Type =
    val lhsType = e.lhs.visit(this)(using context)
    val rhsType = e.rhs.visit(this)(using context)
    val functionType = e.function.visit(this)(using context)

    Type.Arrow.from(functionType) match
      case None =>
        val fresh = freshTypeVariable()
        context.obligations.add(Constraint.Apply(functionType, List(Labeled(None, lhsType), Labeled(None, rhsType)), fresh, Constraint.Origin(e.site)))
        context.obligations.constrain(e, fresh)
      case Some(value) =>
        context.obligations.add(Constraint.Apply(functionType, List(Labeled(None, lhsType), Labeled(None, rhsType)), value.output, Constraint.Origin(e.site)))
        context.obligations.constrain(e, value.output)
  ...
  def visitConditional(e: ast.Conditional)(using context: Typer.Context): Type =
    ...
    val thenBranch = e.successCase.visit(this)
    val elseBranch = e.failureCase.visit(this)

    if thenBranch == elseBranch then
      context.obligations.constrain(e, thenBranch)
    else
      val merge = freshTypeVariable()
      context.obligations.add(Constraint.Subtype(thenBranch, merge, Constraint.Origin(e.successCase.site)))
      context.obligations.add(Constraint.Subtype(elseBranch, merge, Constraint.Origin(e.failureCase.site)))
  ...
  def visitLet(e: ast.Let)(using context: Typer.Context): Type =
    ...
    context.inScope(e, { (inner) =>
      val binding = e.binding.visit(this)(using inner)
      val body = e.body.visit(this)(using inner)
      context.obligations.add(Constraint.Subtype(binding, body, Constraint.Origin(e.site)))
  ...
  def visitLambda(e: ast.Lambda)(using context: Typer.Context): Type =
    ...
    context.inScope(e, { (inner) =>
      ...
      val inputTypes = computedUncheckedInputTypes(e.inputs)
      val bodyType = e.body.visit(this)(using inner)

      e.output match
        case None =>
          val fresh = freshTypeVariable()
          context.obligations.add(Constraint.Subtype(bodyType, fresh, Constraint.Origin(e.body.site)))
          context.obligations.constrain(e, Type.Arrow(inputTypes, fresh))
        case Some(outputType) =>
          val visitedType = outputType.visit(this)(using inner)
          context.obligations.add(Constraint.Subtype(bodyType, visitedType, Constraint.Origin(e.body.site)))
          context.obligations.constrain(e, Type.Arrow(inputTypes, visitedType))
    ...
    val result = evaluateTypeTree(e.ascription) match
      ...
      case ascription =>
        e.operation match
          case Typecast.Widen => 
            val innerType = e.inner.visit(this)
            context.obligations.add(Constraint.Subtype(innerType, ascription, Constraint.Origin(e.site)))
            ascription
          case Typecast.Narrow =>
            val innerType = e.inner.visit(this)
            context.obligations.add(Constraint.Subtype(ascription, innerType, Constraint.Origin(e.site)))
            return context.obligations.constrain(e, Type.some(ascription))
          case Typecast.NarrowUnconditionally =>
            checkedTypeEnsuringConvertible(e.inner, ascription) match
  ...
  def visitTypeIdentifier(e: ast.TypeIdentifier)(using context: Typer.Context): Type =
      lookupUnqualified(e.value) match
        case Nil =>
          report(TypeError(s"undefined type '${e.value}'", e.site))
          Type.Error
        case List(singleType) =>
          singleType.tpe match
            case Type.Meta(t) => t 
            case _ => 
              report(TypeError(s"ambiguous use of type '${e.value}'", e.site))
              Type.Error
        case _ =>
          report(TypeError(s"ambiguous use of type '${e.value}'", e.site))
          Type.Error 
  ...
  def visitArrow(e: ast.Arrow)(using context: Typer.Context): Type =
    val inputs = e.inputs.map(l => Labeled(l.label, l.value.visit(this)(using context)))
    val output = e.output.visit(this)(using context)
  ...
  def visitRecordPattern(p: ast.RecordPattern)(using context: Typer.Context): Type =
    val fieldsType = p.fields.map(l => Labeled(l.label, l.value.visit(this)))
    context.obligations.constrain(p, Type.Record(p.identifier, fieldsType))
